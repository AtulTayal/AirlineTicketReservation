...
class Tickets extends Component {
    ...
    */
    ...
    cancelTicket = (tickets) => {  
        ...
        if(window.confirm("Are you sure you want to cancel the tickets " + tickets + " ?")) {
            this.service.cancelTicket(tickets).then(response => {
                this.getTickets();
                Swal.fire({
                    position: 'center',
                    icon: 'success',
    ...
    render(){
        ...
        const flightList = this.tickets.map((x)=>
                <tr key={x.ticketNumber}>
                    ...
                    <td><button className="btn btn-outline-info" onClick={() => this.showTicket(x)}>View Ticket</button></td>
                     <td><button className="btn btn-outline-info" onClick={() => this.cancelTicket(x.ticketNumber)}>Cancel Booking</button></td> 
                </tr>
        )
